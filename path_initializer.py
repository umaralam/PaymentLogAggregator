        ...
        try:
            if pname == "CONSUMER_SERVICE" or pname == "PAYCORE_SERVICE"\
                ...
                if self.create_modified_log4j2_xml(pname, log4j2_path):
                    modified_log4j2 = os.path.join(os.getcwd(), "modified_nlog.config")
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        if pname == 'GRIFF':
                                            ...
                                            self.packs_tomcat_log_path_dict["packs_{0}_backup_log".format(logger_ref)] = replacedValue   
                                    else:
                                        if pname == 'GRIFF':
                                            self.griff_tomcat_log_path_dict["griff_{0}_backup_log".format(logger_ref)] = appender.attrib.get('filePattern')
                                        elif pname == 'PACKS':
                                            self.packs_tomcat_log_path_dict["packs_{0}_backup_log".format(logger_ref)] = appender.attrib.get('filePattern')
                                            
                                else:
                                    if pname == 'GRIFF':
                                        self.griff_tomcat_log_path_dict["griff_{0}_log".format(logger_ref)] = appender.attrib.get('fileName')
                                    elif pname == 'PACKS':
                                        self.packs_tomcat_log_path_dict["packs_{0}_log".format(logger_ref)] = appender.attrib.get('fileName')
                ...
                if pname == "CONSUMER_SERVICE":
                            ...
                            if logger_ref == str(appender.attrib.get('name')):
                                for f in appender.findall('file'):
                                    replacedValue = str(f.attrib.get('value'))\
                                                    ...
                                                    .replace("./", "{}/".format(self.onmopay_consumer_process_home_dir))
                                    self.onmopay_consumer_log_path_dict["onmopay_consumer_{0}_log".format(logger_ref)] = replacedValue    
                    ...
                    for appender in tree.findall('./targets/target'):
                        if logger_ref == str(appender.attrib.get('name')):
                            replacedValue = str(appender.attrib.get('fileName'))\
                                            ...
                                            .replace("./", "{}/".format(self.onmopay_paycore_process_home_dir))
                            self.onmopay_paycore_log_path_dict["onmopay_paycore_{0}_log".format(logger_ref)] = replacedValue                           
                    ...
                    for appender in tree.findall('./targets/target'):
                        if logger_ref == str(appender.attrib.get('name')):
                            replacedValue = str(appender.attrib.get('fileName'))\
                                            .replace("${hostname}", self.hostname)\
                                            .replace("./", "{}/".format(self.onmopay_paycore_webapi_process_home_dir))
                            self.onmopay_paycoreWebApi_log_path_dict["onmopay_paycore_webapi_{0}_log".format(logger_ref)] = replacedValue                           
                    ...
                    for appender in tree.findall('./targets/target'):
                        if logger_ref == str(appender.attrib.get('name')):
                            replacedValue = str(appender.attrib.get('fileName'))\
                                            .replace("${hostname}", self.hostname)\
                                            .replace("./", "{}/".format(self.onmopay_callback_delivery_process_home_dir))
                            self.onmopay_callbackDelivery_log_path_dict["onmopay_callback_delivery_{0}_log".format(logger_ref)] = replacedValue                           
                        ...
                        if route.attrib.get('ref') == appender.attrib.get('name'):
                            if (
                                ...
                                if (
                                    ...
                                    if pname == 'GRIFF':
                                        self.griff_tomcat_log_path_dict["griff_{0}_backup_log".format(route.attrib.get('ref'))] = appender.attrib.get('filePattern')   
                                    elif pname == 'PACKS':
                                        self.packs_tomcat_log_path_dict["packs_{0}_backup_log".format(route.attrib.get('ref'))] = appender.attrib.get('filePattern')   
                            else:
                                if pname == 'GRIFF':
    ...
    def is_msisdn_in_debugMsisdn_list(self, pname):
        ...
        if pname == "GRIFF":
            if self.griff_tomcat_log_path_dict[self.griff_process_home_directory]:
                property_directory = "{}/conf/griff/resources".format(self.griff_tomcat_log_path_dict[self.griff_process_home_directory])
                # path = Path("{}/service.properties".format(property_directory))
                path = os.path.join(property_directory, 'service.properties')
                
        elif pname == "PACKS":
            if self.packs_tomcat_log_path_dict[self.packs_process_home_directory]:
                property_directory = "{}/conf/packs".format(self.packs_tomcat_log_path_dict[self.packs_process_home_directory])
                # path = Path("{}/packs.properties".format(property_directory))
                path = os.path.join(property_directory, 'packs.properties')
