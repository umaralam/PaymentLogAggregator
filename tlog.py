...
<<<<<<< HEAD
            if pname == "ONMOPAY" or pname == "ONMOPAY_CG_REDIRECTION":
                ...
                if pname == "ONMOPAY":
                    ctid_mdn_data = subprocess.check_output("grep -a " + str(self.validation_object.fmsisdn) + " " + file + " | cut -d ',' -f 3,4", universal_newlines=True, shell=True, preexec_fn=lambda: signal.signal(signal.SIGPIPE, signal.SIG_DFL))

                elif pname == "ONMOPAY_CG_REDIRECTION":
                    ctid_mdn_data = subprocess.check_output("grep -a " + str(self.validation_object.fmsisdn) + " " + str(file) + " | cut -d ',' -f 5,6", universal_newlines=True, shell=True, preexec_fn=lambda: signal.signal(signal.SIGPIPE, signal.SIG_DFL))
                ...
                ctid_data = subprocess.check_output("cat {} | cut -d ',' -f 2 | sort -u".format(file), shell=True, preexec_fn=lambda: signal.signal(signal.SIGPIPE, signal.SIG_DFL))
                if ctid_data:
                    ...
                    if pname == "ONMOPAY":
                        ...
                        if ctid == splitted_data[1].replace('"', '').strip():
                            ...
                            for index, element in enumerate(splitted_data):
                                data_dict[header[index]] = element.replace('"', '').replace("'", '"')
                            self.ctid_data_dict[ctid].append(data_dict)
                            # logging.info('ctid data dict is: %s', self.ctid_data_dict)
